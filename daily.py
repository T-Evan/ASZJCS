    ...
    def 主线探索(self):
        ...
        for k in range(40):
            ...
            if re:
                Toast('开启探索定位1')
            re = FindColors.find(
                "285,1014,#4F4F4F|271,994,#2C2E2E|281,994,#2C2E2E|267,1034,#2C2D2E|280,1035,#2C2E2E|267,1013,#4F4F4F|276,1014,#4F4F4F",
                rect=[254, 917, 299, 1050])
            if re:
                Toast('开启探索定位2')
                tapSleep(re.x, re.y)
            ...
            if not isFind:
                re, _, _ = imageFind('探索-队伍', x1=14, y1=119, x2=110, y2=200)
                if not re:
                    Toast('返回探索地图')
                    tapSleep(337, 36)
                if re:
