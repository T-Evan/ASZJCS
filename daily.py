...
class DailyTask:
    ...
    def 食材采集(self):
        ...
        if not re:
            Toast('点击小地图')
            tapSleep(612, 173)  # 点击地图

        Toast('缩放小地图')
        tapSleep(683, 1093)  # 缩放地图
        for i in range(4):
            Toast('初始化地图位置')
            swipe(88, 426, 549, 1047)
            sleep(0.4)

        for k in range(6):
            swipe(552, 467, 296, 467)
            sleep(0.7)
            swipe(402, 714, 380, 714)
            re = imageFindClick('龙之国-梢云谷')
            if re:
                Toast('寻找食材')
                tapSleep(72, 1212)  # 返回探索地图
                re = imageFindClick('食材-荠菜')
                if re:
                    Toast('寻找食材-成功')
                break
    ...
    def 月卡领取(self):
        ...
        if not re:
            Toast('未找到月卡入口')
            return
        re, _ = TomatoOcrText(461, 509, 642, 552, '元', match_mode='fuzzy')
        if not re:
            tapSleep(538, 527)  # 领取
            tapSleep(421, 1237)  # 空白

        re, _ = TomatoOcrText(457, 995, 642, 1043, '元', match_mode='fuzzy')
        if not re:
    ...
    def 命运之树领取(self):
        ...
        if not re:
            re = FindColors.find("401,618,#FFBDC7|390,628,#FFC5CD|404,634,#FFB8C3|394,626,#FFCED5",
                                 rect=[28, 460, 713, 826], diff=0.94)
        if re:
    ...
    def 自动主线(self):
        ...
        # 判断是否战力不足
        re = FindColors.find("143,957,#CC7129|141,961,#CE722A|147,961,#D4762A|150,957,#A75D28",
                             rect=[21, 926, 315, 1063], diff=0.93)
        if re:
            Toast('战力不足-跳过主线')
            sleep(2)
            return
        ...
        if not re:
            re = FindColors.find("236,991,#B0B556|241,997,#CBCF5F|235,1005,#CAD060", rect=[12, 893, 317, 1043])
        if re:
    ...
    def 主线探索(self):
        re = FindColors.find("140,942,#E57F2C|147,946,#E57F2A|138,949,#CC702A", rect=[7, 894, 276, 1040], diff=0.9)
        if re:
            Toast('战力未达到探索推荐-跳过探索')
            sleep(2)
        ...
        for k in range(40):
            ...
            if not re:
                ...
                if re:
                    Toast('开启探索定位2')
            if not re:
                re = imageFindClick('探索-定位未开启3', x1=16, y1=894, x2=329, y2=1059, rgb=True, confidence1=0.7)
                if re:
                    Toast('开启探索定位3')
            ...
            if not isFind:
                ...
                if isFind:
                    findX = isFind.x - 10
                    findY = isFind.y + 10
                    if 609 < findX < 701 and 325 < findY < 607:
                        print('主线-侧边活动跳过点击')
                        isFind = False
                    else:
                        Toast('寻路主线任务提交')
            ...
            if not isFind:
                ...
                if not isFind:
                    isFind = FindColors.find(
                        "85,744,#222222|101,743,#CBCF60|120,741,#212121|104,770,#FCFCF3|101,776,#FEFFF2|102,778,#FFFFF5",
                        diff=0.94)
                if isFind:
                    findPointRight = True
                    ...
                    if findY > 1220:  # 避免底部无法直接点击
                        isFind = False
                        print(f'精准寻路{findX},{findY}-忽略底部')
                    if findX > 580 and findY < 100:  # 避免底部无法直接点击
                        isFind = False
                        print(f'精准寻路{findX},{findY}-忽略右上角')
                    findY = findY + 80
            ...
            if isFind:
                if 5 <= lastPointTimes < 6:
                    Toast('寻路方案2')
                    swipe(344, 857, 375, 505)
                    sleep(1)
                    tapSleep(findX, findY + 250, 1.5)  # 点击探索标记下方一格寻路
                    tapSleep(findX, findY + 250, 1.5)  # 点击探索标记下方一格寻路
                if 6 <= lastPointTimes < 7:
                    Toast('寻路方案3')
                    swipe(344, 857, 375, 505)
                    sleep(1)
                    tapSleep(findX - 350, findY - 250, 1.5)  # 点击探索标记下方一格寻路
                    tapSleep(findX - 350, findY - 250, 1.5)  # 点击探索标记下方一格寻路
                ...
                if re:
                    self.角色信息检查()
                    if findPointRight:
                        Toast(f'精准寻路1-第{lastPointTimes}/12次')
                        tapSleep(findX, findY, 0.5)  # 切换探索标记的地图视角
                    else:
                        print('模糊寻路1')
                        Toast(f'模糊寻路1-第{lastPointTimes}/12次')
                        tapSleep(findX, findY, 0.5)  # 切换探索标记的地图视角
                        self.角色信息检查()
                        # print('模糊寻路2')
    ...
    def 地图探索(self):
        ...
        for k in range(120):
            ...
            if not isStart:
                re = imageFindClick('探索-托管', x1=7, y1=779, x2=341, y2=1063, rgb=True, confidence1=0.6)
                if not re:
                    re = imageFindClick('探索-托管2', x1=7, y1=779, x2=341, y2=1063, rgb=True, confidence1=0.6)
                if not re:
                    re = imageFindClick('探索-托管3', x1=7, y1=779, x2=341, y2=1063, rgb=True, confidence1=0.6)
                if re:
                    Toast('探索-配置托管')
                    for i in range(40):
    ...
    def 其他检查(self):
        ...
        if re:
            if 609 < re.x < 701 and 325 < re.y < 607:
                print('主线-侧边活动跳过点击')
            else:
    ...
    def 战斗检查(self):
        ...
        for k in range(90):
            ...
            re = CompareColors.compare("25,918,#CDCF68|30,937,#CDD067|42,924,#E1E1E1|61,923,#222222|74,921,#C4C864",
                                       diff=0.94)  # 快速跳过
            if re:
                Toast('战斗快速跳过')
                tapSleep(48, 921)

            re = CompareColors.compare("60,222,#D2D569|64,246,#D2D569|652,216,#D2D66A|652,263,#CFD368")
            if re:
                Toast('战斗结束-挑战胜利')
                tapSleep(337, 1231)
                break
    ...
    def 对话检查(self):
        re = FindColors.find("354,1241,#202020|366,1241,#202020|360,1252,#222222|352,1251,#DBDBDB|367,1252,#DCDCDC",
                             rect=[320, 1212, 406, 1275], diff=0.98)  # 匹配对话箭头
        if not re:
            re = FindColors.find("339,1252,#DCDCDC|360,1252,#222222|380,1254,#DCDCDC|361,1259,#252525|360,1268,#E1E1E1",
                                 rect=[290, 1231, 420, 1276], diff=0.95)
        if re:
            failTimes = 0
            for k in range(30):
                re = FindColors.find(
